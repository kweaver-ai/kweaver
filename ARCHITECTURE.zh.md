...
- **前端采用 Monorepo**：DIP 主应用、共享组件库、微前端模块（含 ISF 前端组件）统一在一个 monorepo 内协作。
...
- **DIP 模块后端**（必须具备独立领域数据与业务逻辑）
  - 允许模块：ChatData / AI Store / 数据语义治理（新增模块需经评审确认）
...
- **Core 无 UI**：Core repo 不出现 React/Vue/静态资源/页面路由/Web Console
  - 例外（仅 ISF）：ISF 的独立前端组件允许存在，但必须由 DIP 统一挂载
...
- **DIP 模块后端**：DIP 内能力模块的后端服务（仅限 ChatData / AI Store / 数据语义治理及评审确认模块），必须具备独立领域数据与业务逻辑。
