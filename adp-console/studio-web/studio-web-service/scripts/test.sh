...
abspath=$(pwd)
source "$abspath/StudioWebService/scripts/tools.sh"
...
mkdir -p /root/.ssh/
rm -rf /root/.ssh/*
cp $abspath/StudioWebService/scripts/config /root/.ssh/
...
-v /root/.ssh/:/root/.ssh/ \
-v $abspath/:/test/ \
acr.aishu.cn/public/studio-web-service-builder-node:18.7.0-20250422 \
/bin/bash /test/StudioWebService/scripts/ut.sh
