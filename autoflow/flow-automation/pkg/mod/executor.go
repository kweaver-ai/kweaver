...
func (e *DefExecutor) initWorkerTask(dagIns *entity.DagInstance, taskIns *entity.TaskInstance) {
	...
	dagIns.ShareData.Save = func(data *entity.ShareData) error {
		patch := &entity.DagInstance{BaseInfo: entity.BaseInfo{ID: taskIns.DagInsID}, DagID: dagIns.DagID, ShareData: data}
		if err := patch.SaveExtData(context.Background()); err != nil {
			return err
		}
		return GetStore().PatchDagIns(context.Background(), patch)
	}
	executeMethods := entity.ExecuteMethods{Publish: NewMQHandler().Publish, GetDag: dag}
	taskIns.InitialDep(
...
func (e *DefExecutor) runActionWithRetry(ctx context.Context, taskIns *entity.TaskInstance, params interface{}, act entity.Action) error {
	...
	if strings.Contains(act.Name(), "@anyshare") ||
		act.Name() == common.MannualTrigger ||
		act.Name() == common.CronTrigger ||
		act.Name() == common.CronWeekTrigger ||
		act.Name() == common.CronMonthTrigger ||
		act.Name() == common.CronCustomTrigger ||
		act.Name() == common.InternalToolPy3Opt ||
		act.Name() == common.AnyshareFileOCROpt ||
		act.Name() == common.AudioTransfer ||
		act.Name() == common.DocInfoEntityExtract ||
		act.Name() == "@custom" ||
		act.Name() == "@operator" ||
		act.Name() == common.OperatorTrigger ||
		act.Name() == common.DataflowDocTrigger ||
		act.Name() == common.DataflowUserTrigger ||
		act.Name() == common.DataflowDeptTrigger ||
		act.Name() == common.DataflowTagTrigger ||
		act.Name() == common.OpAnyDataCallAgent ||
		act.Name() == common.OpLLMChatCompletion ||
		act.Name() == common.OpContentEntity ||
		act.Name() == common.OpEcoconfigReindex ||
		act.Name() == common.DatabaseWriteOpt ||
		act.Name() == common.OpLLMReranker ||
		act.Name() == common.OpLLmEmbedding ||
		act.Name() == common.OpContentFileParse {
		retryCount := 0
		waitTime := 3 * time.Second
		for {
			var tokenInfo *entity.Token

			if taskIns.RelatedDagInstance.Trigger == entity.TriggerSecurityPolicy {
				...
				if exists {
					if m, ok := source.(map[string]interface{}); ok {
						if m["type"] == "file" || m["type"] == "folder" {
							source_id = m["id"].(string)
						}
					}
				}

				if source_id != "" {
					var owners []drivenadapters.DocOwner
					docshareAdapters := drivenadapters.NewDocShare()
					owners, err = docshareAdapters.GetDocOwners(ctx, source_id)
					...
					userids := []string{
						taskIns.RelatedDagInstance.Vars["userid"].Value,
					}

					for _, docOwner := range owners {
						if docOwner.Owner.Type == "app" {
							continue
						}
						ownerMap[docOwner.Owner.ID] = docOwner.Owner
						userids = append([]string{docOwner.Owner.ID}, userids...)
					}

					workflowApprovalTaskIds, ok := taskIns.RelatedDagInstance.ShareData.Get(common.WorkflowApprovalTaskIds)

					if ok {
						if taskIds, ok := workflowApprovalTaskIds.(primitive.A); ok && len(taskIds) > 0 {
					...
					userid := taskIns.RelatedDagInstance.Vars["userid"].Value
					tokenMgnt := NewTokenMgnt(userid)
					tokenInfo, err = tokenMgnt.GetAppToken()
					tokenInfo.IsApp = true
				}
			} else if act.Name() == common.AnyshareDocLibQuotaScaleOpt {
				// 获取任务执行所需要的token
				...
				appInfo := taskIns.RelatedDagInstance.AppInfo
				if appInfo.Enable {
			...
			if tokenInfo == nil {
				return ierrors.NewIError(ierrors.UnAuthorization, "", map[string]interface{}{"info": "get token failed"})
			}

			err = taskIns.Run(ctx, params, act, tokenInfo)

			// 如果是配额空间扩容节点，非流程管理员则不重试
	...
	if act.Name() == common.Loop {
		...
		if actualLimit > 0 && loopParams.CurrentIteration >= actualLimit {
			...
			// GetParser().EntryTaskIns(taskIns)
			// 只在最后一个循环节点收集输出
			if err := e.collectLoopOutputs(ctx, taskIns, &loopParams, taskIns.RelatedDagInstance.ShareData); err != nil {
	...
	err = taskIns.Run(ctx, params, act, nil)
	return ierrors.ParseError(err)
}
...
// collectLoopOutputs collects outputs from all loop iterations
func (e *DefExecutor) collectLoopOutputs(ctx context.Context, taskIns *entity.TaskInstance, loopParams *LoopParameters, shareData *entity.ShareData) error {
