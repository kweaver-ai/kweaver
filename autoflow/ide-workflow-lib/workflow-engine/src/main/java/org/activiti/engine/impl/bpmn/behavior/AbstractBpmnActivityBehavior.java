...
public class AbstractBpmnActivityBehavior extends FlowNodeActivityBehavior {
  ...
  protected void leave(ActivityExecution execution) {
    ...
    if (!hasLoopCharacteristics()) {
      ...
      //by lw
      leaveChangeActivityBehavior(execution,(ActivityImpl) execution.getActivity());
  ...
  /**
	 ...
	 * @param activity
	 */
	private void leaveChangeActivityBehavior(ActivityExecution execution, ActivityImpl activity) {
		ActivityBehavior activityBehavior = activity.getActivityBehavior();
		if (StringUtils.isNotEmpty(execution.getBusinessKey())&&execution.getBusinessKey().equals("zjsh")) {
			...
			this.multiInstanceActivityBehavior=sqmult;
		}
		if(execution.getBusinessKey()==null) {
			activity.setProperty("dealType", "tjsh");
		}else {
			activity.setProperty("dealType", execution.getBusinessKey());
		}
	}
