...
public class InclusiveGatewayActivityBehavior extends GatewayActivityBehavior {
		...
		if (!activeConcurrentExecutionsExist(execution)) {
			...
			for (PvmTransition outgoingTransition : execution.getActivity().getOutgoingTransitions()) {
				...
				if (!SkipExpressionUtil.isSkipExpressionEnabled(execution, skipExpression)) {
					if (defaultSequenceFlow == null || !outgoingTransition.getId().equals(defaultSequenceFlow)) {
						 ...
						 * outgoingTransition.getProperty(BpmnParse.PROPERTYNAME_CONDITION); if
						 * (condition == null || condition.evaluate(outgoingTransition.getId(),
						 * execution)) { transitionsToTake.add(outgoingTransition); }
						 */
						// by lw
						if (!processDefinitionResource.filterActivity(execution.getProcessDefinitionId(),
								(ActivityImpl) outgoingTransition.getSource(),
								(ActivityImpl) outgoingTransition.getDestination(), execution.getVariables())) {
			...
			try {
				TaskEntity task = TaskEntity
						.create(Context.getProcessEngineConfiguration().getClock().getCurrentTime());
				// 绑定任务扩展属性
				WfEngineUtils.buildExtTaskEntity(task, (ExecutionEntity) execution);
				HistoricTaskInstanceEntity historicTaskInstance = new HistoricTaskInstanceEntity(task,
				...
				Context.getCommandContext().getCommentEntityManager().addCommentNotValidate(
						historicTaskInstance.getPreTaskId(), historicTaskInstance.getProcessInstanceId(),
						((ExecutionEntity) execution).getTopProcessInstanceId(), processInputModel.getWf_curComment(),
						processInputModel.getWf_commentDisplayArea());

				// Context.getCommandContext().getHistoryManager().recordActivityEndExtend(historicTaskInstance,(String)
				// execution.getActivity().getProperty("type"));
			} catch (Exception e) {
				log.error("Inclusive gateway save historicTaskInstance error", e);
			}
		}
	}
			...
			for (ActivityExecution concurrentExecution : getLeaveExecutions(execution.getParent())) {
				if (concurrentExecution.isActive() && concurrentExecution.getId().equals(execution.getId()) == false) {
					...
					if (pvmTransition != null) {
						...
						reachable = isReachable(concurrentExecution.getActivity(), activity,
								new HashSet<PvmActivity>());
					}
	...
	protected boolean isReachable(PvmActivity srcActivity, PvmActivity targetActivity,
		...
		// if source has no outputs, it is the end of the process, and its parent
