...
public class AddIdentityLinkCmd extends NeedsActiveTaskCmd<Void> {
  ...
  private Boolean notUseTransferDscription = false;
  ...
  public AddIdentityLinkCmd(String taskId, String userId, String groupId, String type, Boolean notUseTransferDscription) {
    super(taskId);
    validateParams(userId, groupId, type, taskId);
    this.taskId = taskId;
    this.userId = userId;
    this.groupId = groupId;
    this.type = type;
    this.notUseTransferDscription = notUseTransferDscription;
  }
  ...
  protected Void execute(CommandContext commandContext, TaskEntity task) {
	boolean isTransfer = false;// bylw 是否转办逻辑
    boolean assignedToNoOne = false;
    if (task.getAssignee() != null) {
      // by siyu.chen 2023/7/25 start
      isTransfer = true;
      task.setActionType(WorkFlowContants.ACTION_TYPE_RECEIVER_TRANSFER);
      if (!this.notUseTransferDscription) {
        String remark="";
        if(task.getDescription()!=null) {
          remark=task.getDescription();
        }
        task.setDescription(remark+""+task.getAssignee()+"转派给"+userId+",");
      }
      // by siyu.chen 2023/7/25 end
    }
 ...
 // by lw   委托代办设置owner时不走通知逻辑
 		if (isTransfer&&!IdentityLinkType.OWNER.equals(type)) {
 			commandContext.getEventDispatcher().dispatchEvent(
 					ActivitiEventBuilder.createEntityEvent(
 							ActivitiEventType.TASK_ASSIGNED_TRANSFER, task));
 		}
    return null;
  }
