...
public class GetProcessInstanceCommentsCmd implements Command<List<Comment>>, Serializable {
	...
	public GetProcessInstanceCommentsCmd(String processInstanceId, String topProcessInstanceId, String type) {
		this.processInstanceId = processInstanceId;
	...
	public List<Comment> execute(CommandContext commandContext) {
		if (StringUtils.isNotBlank(topProcessInstanceId)) {
			List<Comment> commentsByProcessInstanceId = commandContext.getCommentEntityManager()
					.findCommentsByTopProcessInstanceId(topProcessInstanceId);
