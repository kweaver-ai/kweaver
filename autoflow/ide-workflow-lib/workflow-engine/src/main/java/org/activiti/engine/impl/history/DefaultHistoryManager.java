...
public class DefaultHistoryManager extends AbstractManager implements HistoryManager {
  ...
  public void recordProcessInstanceEnd(String processInstanceId, String deleteReason, String activityId) {
    ...
    if(isHistoryLevelAtLeast(HistoryLevel.ACTIVITY)) {
      ...
      if (historicProcessInstance!=null) {
        ...
        //by lw
        historicProcessInstance.setProcState(SuspensionState.FINISH
				.getStateCode());
        // Fire event
  ...
  @Override
  public void recordProcessInstanceAutoEnd(String processInstanceId, String deleteReason, String activityId) {
    ...
    if(isHistoryLevelAtLeast(HistoryLevel.ACTIVITY)) {
      ...
      if (historicProcessInstance!=null) {
        ...
        historicProcessInstance.setEndActivityId(activityId);
        //by lw
        historicProcessInstance.setProcState(SuspensionState.AUTO_REJECT
				.getStateCode());
        // Fire event
        ProcessEngineConfigurationImpl config = Context.getProcessEngineConfiguration();
    		if (config != null && config.getEventDispatcher().isEnabled()) {
  ...
  public void recordProcessInstanceStart(ExecutionEntity processInstance) {
    if(isHistoryLevelAtLeast(HistoryLevel.ACTIVITY)) {
      ...
      //by lw
   	  WfEngineUtils.buildExtActivityInstance(historicActivityInstance, processInstance);
  ...
  public void recordSubProcessInstanceStart(ExecutionEntity parentExecution, ExecutionEntity subProcessInstance) {
    if(isHistoryLevelAtLeast(HistoryLevel.ACTIVITY)) {
      ...
      historicActivityInstance.setStartTime(now);
   //by lw
   	  WfEngineUtils.buildExtSubActivityInstance(
   					historicActivityInstance, parentExecution);
      getDbSqlSession().insert(historicActivityInstance);
...
public void recordActivityStart(ExecutionEntity executionEntity) {
    if(isHistoryLevelAtLeast(HistoryLevel.ACTIVITY)) {
     ...
     // 2014-1-28 by lw
		WfEngineUtils.buildExtActivityInstance(
				historicActivityInstance, executionEntity);
    		getDbSqlSession().insert(historicActivityInstance);
  ...
  public void recordActivityEnd(ExecutionEntity executionEntity) {
    if(isHistoryLevelAtLeast(HistoryLevel.ACTIVITY)) {
      ...
      //by lw
		/*
		 * try{
		 * updatePrevHisTaskInstanceEndTime(executionEntity,historicActivityInstance);
		 * }catch(Exception e){
		 * this.log.error("updatePrevHisTaskInstanceEndTime error",e); }
		 */
    }
  }
  ...
  public void recordStartEventEnded(ExecutionEntity execution, String activityId) {
    if(isHistoryLevelAtLeast(HistoryLevel.ACTIVITY)) {
      ...
      if (historicActivityInstance != null) {
		...
		/*
		 * List<ExecutionEntity> executionEntitys = getDbSqlSession()
		 * .findInCache(ExecutionEntity.class); // Search for the historic activity
		 * instance in the dbsqlsession // cache, since process hasn't been persisted to
		 * db yet List<HistoricActivityInstanceEntity> cachedHistoricActivityInstances =
		 * getDbSqlSession() .findInCache(HistoricActivityInstanceEntity.class); for
		 * (HistoricActivityInstanceEntity cachedHistoricActivityInstance :
		 * cachedHistoricActivityInstances) { if
		 ...
		 * cachedHistoricActivityInstance.markEnded(null); // by lw if (executionEntitys
		 * != null && !executionEntitys.isEmpty()) {
		 * WfEngineUtils.buildExtActivityInstance( cachedHistoricActivityInstance,
		 * executionEntitys.get(0)); } return; } }
		 */
    }
  }
...
public void recordTaskClaim(String taskId) {
  ...
  */
  @Override
  public void recordTaskEnd(ExecutionEntity execution,String taskId, String deleteReason) {
    if (isHistoryLevelAtLeast(HistoryLevel.AUDIT)) {
      HistoricTaskInstanceEntity historicTaskInstance = getDbSqlSession().selectById(HistoricTaskInstanceEntity.class, taskId);
      if (historicTaskInstance!=null) {
  ...
  */
  ...
  public void recordVariableCreate(VariableInstanceEntity variable) {
    // Historic variables 
	  //by lw
	  if (isHistoryLevelAtLeast(HistoryLevel.ACTIVITY)) {
		  if((variable.getName()!=null&&!(variable.getName().indexOf("wf_pre")!=-1))) {
  ...
  */
  ...
  /*
	 ...
	 * by lw add assigneeUserName params
	 */
	@Override
	public void recordTaskAssigneeChange(TaskEntity task) {
		if (isHistoryLevelAtLeast(HistoryLevel.AUDIT)) {
			...
			if (historicTaskInstance != null) {
				...
				historicTaskInstance.setAssigneeOrgId(task.getAssigneeOrgId());
				historicTaskInstance.setAssigneeOrgName(task.getAssigneeOrgName());
				historicTaskInstance.setAssigneeUserName(task.getAssigneeUserName());
			}
		}
	}
	 ...
	 * enabled.
	 */
	public HistoricTaskInstanceEntity recordTaskEndExtend(
		...
		if (isHistoryLevelAtLeast(HistoryLevel.AUDIT)) {
			...
			historicTaskInstance = new HistoricTaskInstanceEntity(task,
					executionEntity);
			ProcessInputModel processInputModel = WfEngineUtils
					.getWfprocessInputModel(execution.getVariables());

			if (processInputModel.getWf_receiver() != null
					&& processInputModel.getWf_receiver().indexOf(",") == -1) {
				task.setAssigneeWithoutCascade(processInputModel
						.getWf_receiver());
			}
			WfEngineUtils.buildExtHistoricTaskInstanceEntity(
					historicTaskInstance, task, executionEntity);
			if (processInputModel.getWf_receiver() != null
					&& processInputModel.getWf_receiver().length() > 100) {
				historicTaskInstance.setAssignee(processInputModel
						.getWf_receiver().substring(0, 100));
			} else {
				historicTaskInstance.setAssignee(processInputModel
						.getWf_receiver());
			}
			if ("callActivity".equals(executionEntity.getActivity()
					.getProperty("type"))) {
				historicTaskInstance.setTaskDefinitionKey(executionEntity
						.getCurrentActivityId());
				historicTaskInstance.setName(executionEntity
						.getCurrentActivityName());
			} else {
				historicTaskInstance.setTaskDefinitionKey(processInputModel
						.getWf_nextActDefId());
				historicTaskInstance.setName(processInputModel
						.getWf_nextActDefName());
			...
			historicTaskInstance.setDescription(processInputModel
					.getWf_nextActDefType());
			historicTaskInstance.setActionType("execute_activity");
			historicTaskInstance.setStartTime(Context
					.getProcessEngineConfiguration().getClock()
					.getCurrentTime());
			...
			Context.getCommandContext()
					...
					.addCommentNotValidate(historicTaskInstance.getPreTaskId(),
							historicTaskInstance.getProcessInstanceId(),executionEntity.getTopProcessInstanceId(),
	 ...
	 * //2014-1-28 by lw
	 */
	public void recordTaskUpdate(TaskEntity task) {
		if (isHistoryLevelAtLeast(HistoryLevel.AUDIT)) {
			HistoricTaskInstanceEntity historicTaskInstance = getDbSqlSession()
					.selectById(HistoricTaskInstanceEntity.class, task.getId());
			if (historicTaskInstance != null) {
				historicTaskInstance.setActionType(task.getActionType());
	 ...
	 * 2014-1-28 by lw
	 */
	public void recordProcessInstanceCancel(String processInstanceId,
			String cancelReason, String activityId) {
		...
		if (isHistoryLevelAtLeast(HistoryLevel.ACTIVITY)) {
			...
			if (historicProcessInstance != null) {
				historicProcessInstance.markEnded(cancelReason);
				historicProcessInstance.setEndActivityId(activityId);
				historicProcessInstance.setProcState(SuspensionState.CANCELED
						.getStateCode());
			}
		}
	}
	...
	@Override
	public void recordActivityEndExtend(HistoricTaskInstanceEntity historicTaskInstance,String activityType) {
		if (isHistoryLevelAtLeast(HistoryLevel.ACTIVITY)) {
				HistoricActivityInstanceEntity historicActivityInstance = new HistoricActivityInstanceEntity();
				historicActivityInstance.setId(historicTaskInstance.getId());
				historicActivityInstance.setProcessDefinitionId(historicTaskInstance.getProcessDefinitionId());
				historicActivityInstance.setProcessDefinitionName(historicTaskInstance.getProcessDefinitionName());
				historicActivityInstance.setProcessInstanceId(historicTaskInstance.getProcessInstanceId());
				historicActivityInstance.setExecutionId(historicTaskInstance.getExecutionId());
				historicActivityInstance.setActivityId(historicTaskInstance.getTaskDefinitionKey());
				historicActivityInstance.setActivityName(historicTaskInstance.getName());
				historicActivityInstance.setActivityType(activityType);
				historicActivityInstance.setStartTime(Context
						.getProcessEngineConfiguration().getClock()
						.getCurrentTime());
				historicActivityInstance.setAssignee(historicTaskInstance.getAssignee());
				historicActivityInstance.setAssigneeOrgId(historicTaskInstance.getAssigneeOrgId());
				historicActivityInstance.setAssigneeOrgName(historicTaskInstance.getAssigneeOrgName());
				historicActivityInstance.setAssigneeUserName(historicTaskInstance.getAssigneeUserName());
				historicActivityInstance.setPreActId(historicTaskInstance.getPreTaskDefKey());
				historicActivityInstance.setPreActInstId(historicTaskInstance.getPreTaskId());
				historicActivityInstance.setPreActName(historicTaskInstance.getPreTaskDefName());
				historicActivityInstance.setProcTitle(historicTaskInstance.getProcTitle());
				historicActivityInstance.setSender(historicTaskInstance.getSender());
				historicActivityInstance.setSenderOrgId(historicTaskInstance.getSenderOrgId());
				historicActivityInstance.setSenderOrgName(historicTaskInstance.getSenderOrgName());
				historicActivityInstance.setSendUserId(historicTaskInstance.getSendUserId());
				historicActivityInstance.setSendUserName(historicTaskInstance.getSendUserName());
				historicActivityInstance.setTaskId(historicTaskInstance.getId());
				historicActivityInstance.setTenantId(historicTaskInstance.getTenantId());
				historicActivityInstance.markEnded("completed");
				getDbSqlSession().insert(historicActivityInstance);
			}
		}
	//by lw
	@Override
	public void recordSubProcessActivityStart(ExecutionEntity executionEntity,
			ActivityImpl startAct) {
		...
		historicActivityInstance.setExecutionId(executionId);
		historicActivityInstance.setActivityId(startAct.getId());
		historicActivityInstance
				.setActivityName((String) startAct
						.getProperty("name"));
		historicActivityInstance
				.setActivityType((String) startAct
						.getProperty("type"));
		historicActivityInstance.setStartTime(Context
				.getProcessEngineConfiguration().getClock()
				.getCurrentTime());
	...
	//by lw
	private Object getToptExecutionVariable(ExecutionEntity executionEntity,
			String variableName) {
		Object obj = executionEntity.getVariableLocal(variableName);
		if (obj != null) {
			return obj;
		}
		ExecutionEntity parent = executionEntity.getParent();
		if (parent != null) {
			obj = getToptExecutionVariable(parent, variableName);
		}
		return obj;
	}
	
	/**
	 * a-b(多实例)-c,c处理完需要更新b环节所有实例的完成时间
	 * @param executionEntity
	 * @param historicActivityInstance
	 */
	private void updatePrevHisTaskInstanceEndTime(
			ExecutionEntity executionEntity,
			HistoricActivityInstanceEntity historicActivityInstance) {
		if(historicActivityInstance==null||StringUtils.isEmpty(historicActivityInstance.getPreActId())) {
			return;
		}
		ActivityImpl act = executionEntity.getProcessDefinition().findActivity(
				historicActivityInstance.getPreActId());
		//上一环节为空不做处理
		if (act == null){
			return;
		}
		
		if(StringUtils.isEmpty((String) act.getProperty("multiInstance"))
				|| !((String) act.getProperty("type")).equals("userTask")) {
			if(act.getParentActivity()==null){
				return;
			}
			//过滤掉上一环节为嵌套子流程的情况
			ActivityImpl subActivity = act.getParentActivity();
			if(subActivity==null||!BpmnXMLConstants.ELEMENT_SUBPROCESS
						.equals(subActivity.getProperty("type"))){
				return ;
			}
		}
		
		String sql = "select * from t_wf_hi_taskinst t where t.proc_inst_id_='"
				+ executionEntity.getProcessInstanceId()
				+ "' and task_def_key_='"
				+ historicActivityInstance.getActivityId()
				+ "' and delete_reason_='custom-completed' and end_time_ is null ";
		List<HistoricTaskInstance> prevHisTasks = new NativeHistoricTaskInstanceQueryImpl(
				Context.getCommandContext()).sql(sql).list();
		for (HistoricTaskInstance historicTaskInstance2 : prevHisTasks) {
			// if(!historicActivityInstance.getId().equals(historicTaskInstance2.getId()))
			HistoricTaskInstanceEntity entity = (HistoricTaskInstanceEntity) historicTaskInstance2;
			entity.setEndTime(historicActivityInstance.getEndTime());
			entity.setDurationInMillis(entity.getEndTime().getTime()
