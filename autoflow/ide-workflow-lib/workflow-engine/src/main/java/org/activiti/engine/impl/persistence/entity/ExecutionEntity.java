...
public class ExecutionEntity extends VariableScopeImpl implements ActivityExecution, ExecutionListenerExecution,
	 ...
	 * <li>an execution has arrived in a parallel gateway or join and that join has
	 * not yet activated/fired.</li>
	 ...
	 * process. so sequentially processing the operations drives the interpretation
	...
	public ExecutionEntity createExecution() {
		...
		// 2014-1-28 by lw
		WfEngineUtils.buildExtExecution(getVariables(), createdExecution);
	...
	public PvmProcessInstance createSubProcessInstance(PvmProcessDefinition processDefinition) {
		...
		// 2014-1-28 by lw
		WfEngineUtils.buildExtExecution(getVariables(), subProcessInstance);
		if (Context.getProcessEngineConfiguration() != null
				&& Context.getProcessEngineConfiguration().getEventDispatcher().isEnabled()) {
			Context.getProcessEngineConfiguration().getEventDispatcher().dispatchEvent(
			...
			if (!isUserTask && Context.getProcessEngineConfiguration() != null
				...
				Context.getProcessEngineConfiguration().getEventDispatcher()
						.dispatchEvent(ActivitiEventBuilder.createSignalEvent(ActivitiEventType.ACTIVITY_SIGNALED,
								signalledActivityId, signalName, signalData, this.id, this.processInstanceId,
	...
	/**
	 * @param fireActivityCompletionEvent This method can be called from other
	 *                                    places (like
	 *                                    {@link #takeAll(List, List)}), where the
	 *                                    event is already fired. In that case,
	 *                                    false is passed an no second event is
	 *                                    fired.
	 */
	public void take(PvmTransition transition, boolean fireActivityCompletionEvent) {
		...
		//by lw 按前端指定下一环节跳转
				String nextActDefId = (String) Context.getCommandContext()
						.getAttribute("nextActDefId");
				if (StringUtils.isNotEmpty(nextActDefId)&&"userTask".equals(getActivity().getProperty("type"))) {
					TransitionImpl newTransition = new TransitionImpl(null,null,
							this.getProcessDefinition());
					newTransition.setSource((ActivityImpl) transition.getSource());
					newTransition.setDestinationByExtend(processDefinition.findActivity(nextActDefId));
	...
	public void takeAll(List<PvmTransition> transitions, List<ActivityExecution> recyclableExecutions) {
		...
		// 2014-1-28 by lw
				boolean isParentExecution = (isConcurrent && !isScope);
				ExecutionEntity concurrentRoot = isParentExecution ? getParent() : this;
				if (isParentExecution) {
					concurrentRoot.setName(name);
					concurrentRoot.setSender(sender);
					concurrentRoot.setProcessDefinitionName(processDefinitionName!=null?processDefinitionName:this.getProcessDefinition().getName());
					concurrentRoot.setSendUserName(sendUserName);
					concurrentRoot.setVariables(this.getVariablesLocal());
				}
		...
		if ((transitions.size() == 1) && (concurrentActiveExecutions.isEmpty())
			...
			for (ExecutionEntity prunedExecution : recyclableExecutionImpls) {
				...
				// never ended as the combination of {activityId,executionId} is not valid
				// anymor
			...
			log.debug("activating the concurrent root {} as the single path of execution going forward",
					concurrentRoot);
			...
			while (!transitions.isEmpty()) {
				...
				if (recyclableExecutions.isEmpty()) {
					...
					log.debug("new {} with parent {} created to take transition {}", outgoingExecution,
							outgoingExecution.getParent(), outgoingTransition);
				} else {
		...
		if (Context.getProcessEngineConfiguration() != null
			...
			Context.getProcessEngineConfiguration().getEventDispatcher()
					.dispatchEvent(ActivitiEventBuilder.createActivityEvent(ActivitiEventType.ACTIVITY_COMPLETED,
							...
							getActivity() != null ? getActivity().getActivityBehavior().getClass().getCanonicalName()
									: null));
		}
	}
	...
	private String getTopDeleteReason(ExecutionEntity executionEntity) {
		if(executionEntity!=null) {
			ExecutionEntity parentExecutionEntity=executionEntity.getParent();
			while(parentExecutionEntity!=null) {
				if(parentExecutionEntity.getDeleteReason()==null) {
					String deleteReason=getTopDeleteReason(parentExecutionEntity);
					if(deleteReason!=null) {
						return deleteReason;
					}
				}
				return parentExecutionEntity.getDeleteReason();
			}
		}
		return "";
	}
	public void remove() {
		...
		deleteVariablesInstanceForLeavingScope();
		this.deleteReason=getTopDeleteReason(this);
		// delete all the tasks
		removeTasks(this.deleteReason);
		...
		for (TaskEntity task : getTasks()) {
			if (replacedBy != null) {
				if (task.getExecution() == null || task.getExecution() != replacedBy) {
					// All tasks should have been moved when "replacedBy" has been set. Just in case
					// tasks where added,
	...
	protected List<VariableInstanceEntity> getSpecificVariables(Collection<String> variableNames) {
		...
		return commandContext.getVariableInstanceEntityManager().findVariableInstancesByExecutionAndNames(id,
				variableNames);
	}
	...
	/**
	 * Adds an IdentityLink for this user with the specified type, but only if the
	 * user is not associated with this instance yet.
	 **/
	...
	public int getCachedEntityState() {
		...
		// Only mark a flag as false when the list is not-null and empty. If null, we
		// can't be sure there are no entries in it since
	...
	//by lw
	public String getTopProcessInstanceId(ExecutionEntity curExecutionEntity){
		String topProcessInstanceId=curExecutionEntity.getTopProcessInstanceId();
		if(StringUtils.isNotEmpty(topProcessInstanceId)){
			return topProcessInstanceId;
		}
		ExecutionEntity topExecutionEntity=curExecutionEntity;
		if(StringUtils.isNotEmpty(curExecutionEntity.getSuperExecutionId())){
			ExecutionEntity superExecution=curExecutionEntity.getSuperExecution();
			if(superExecution!=null){
				return getTopProcessInstanceId(superExecution);
			}
			topExecutionEntity=superExecution;
		}if(topExecutionEntity.getParent()!=null){
			ExecutionEntity superExecution=topExecutionEntity.getParent();
			if(superExecution!=null){
				return getTopProcessInstanceId(superExecution);
			}
			topExecutionEntity=superExecution;
		}
		topProcessInstanceId=topExecutionEntity.getProcessInstanceId();
		return topProcessInstanceId;
	}
