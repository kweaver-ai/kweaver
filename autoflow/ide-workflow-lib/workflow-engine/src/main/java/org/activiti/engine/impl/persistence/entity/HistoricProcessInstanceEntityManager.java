...
public class HistoricProcessInstanceEntityManager extends AbstractManager {
  ...
  //by lw
  public List<HistoricProcessInstance> findAllHistoricProcessInstances(HistoricProcessInstanceQueryImpl query) {
		if (getHistoryManager().isHistoryEnabled()) {
			return getDbSqlSession().selectList("selectAllHistoricProcessInstance", query);
		}
		return Collections.EMPTY_LIST;
	}
}
