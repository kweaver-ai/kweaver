...
//2014-1-28 by lw
	...
	protected String bizId,docId,docName,docPath,addition;
	...
	public static TaskEntity createAndInsert(ActivityExecution execution) {
		...
		//by hj
	    if(null != execution.getVariable("customDescription")) {
			JSONObject jsonObj = new JSONObject();
	    	JSONObject customDescriptionJson = JSONUtil.parseObj(execution.getVariable("customDescription"));
			// change by siyu.chen begin
			// 转审配置
			if (customDescriptionJson.get("transfer") != null) {
				JSONObject transferJSON = JSONUtil.parseObj(customDescriptionJson.get("transfer"));
				if ("Y".equals(transferJSON.getStr("transferSwitch"))) {
					jsonObj.set("transfer", transferJSON);
				}
			}
			// 加签配置
	    	String customType = customDescriptionJson.getStr("customType");
	        if("countersign".equals(customType)){
	        	String countersignSwitch = customDescriptionJson.getStr("countersignSwitch");
	        	if("Y".equals(countersignSwitch)) {
					jsonObj.set("customType", "countersign");
					jsonObj.set("countersignSwitch", countersignSwitch);
					jsonObj.set("maxCount", customDescriptionJson.getStr("maxCount"));
					jsonObj.set("maxAuditors", customDescriptionJson.getStr("maxAuditors"));
	        		// task.setDescription((String) execution.getVariable("customDescription"));
		    	}
	        }

			String sendBackSwitch = customDescriptionJson.getStr("sendBackSwitch");
			if ( sendBackSwitch != null && "Y".equals(sendBackSwitch) ) {
				jsonObj.set("sendBackSwitch", sendBackSwitch);
			}
			if (!jsonObj.isEmpty()){
				task.setDescription(JSONUtil.toJsonStr(jsonObj));;
			}
			// change by siyu.chen end
	    }
	    // by lw end
	...
	/**
	 * Creates a new task. Embedded state and create time will be initialized. But
	 * this task still will have to be persisted. See
	...
	public void complete(Map variablesMap, boolean localScope) {
		...
		if (executionId != null) {
			...
			// by lw
			WfEngineUtils.buildExtTaskEntityByComplete(this, execution);
	...
	public void setAssignee(String assignee, boolean dispatchAssignmentEvent, boolean dispatchUpdateEvent) {
		...
		if (assignee == null && this.assignee == null) {
			...
			// ACT-1923: even if assignee is unmodified and null, this should be stored in
			// history.
			if (commandContext != null) {
				/*
				 ...
				 */
				// by lw
				commandContext.getHistoryManager().recordTaskAssigneeChange(this);
			}
		...
		if (commandContext != null) {
			 ...
			 */
			// by lw
			WfEngineUtils.additionalAssignee(this);
			commandContext.getHistoryManager().recordTaskAssigneeChange(this);
...
//    }
	...
	public void fireEvent(String taskEventName) {
		...
		if (taskDefinition != null) {
			...
			// 2014-1-28 by lw
			if (taskEventName.equals(TaskListener.EVENTNAME_CREATE)) {
				List<TaskListener> taskCreateListeners = Context.getProcessEngineConfiguration()
						.getGlobalTaskCreateListeners();
				if (taskCreateListeners != null && !taskCreateListeners.isEmpty())
					for (TaskListener taskListener : taskCreateListeners) {
						executeListener(taskEventName, taskListener);
					}
			} else if (taskEventName.equals(TaskListener.EVENTNAME_COMPLETE)) {
				List<TaskListener> taskCompleteListeners = Context.getProcessEngineConfiguration()
						.getGlobalTaskCompleteListeners();
				if (taskCompleteListeners != null && !taskCompleteListeners.isEmpty())
					for (TaskListener taskListener : taskCompleteListeners) {
						executeListener(taskEventName, taskListener);
					}
			}
		}
...
//2014-1-27 by lw start
	...
	// by lw
	public void setAssigneeNotRecordIdentityLink(String assignee) {
		if (assignee == null && this.assignee == null) {
			return;
		}
		this.assignee = assignee;
	...
	public void updateCascade() {
		...
		DbSqlSession dbSqlSession = commandContext.getDbSqlSession();
		dbSqlSession.update(this);
		Context.getCommandContext().getHistoryManager().recordTaskUpdate(this);
	}

	// 2014-1-28 by lw start
	// 2013/5/16 by lw add orgId
	public IdentityLinkEntity addIdentityLink(String userId, String groupId, String orgId, String type) {
		IdentityLinkEntity identityLinkEntity = new IdentityLinkEntity();
		getIdentityLinks().add(identityLinkEntity);
		identityLinkEntity.setTask(this);
		identityLinkEntity.setUserId(userId);
	...
	public void addCandidateOrg(String orgId) {
		addIdentityLink(null, null, orgId, IdentityLinkType.CANDIDATE);
	}

	public void addCandidateOrgs(Collection<String> candidateOrgs) {
		for (String candidateOrg : candidateOrgs) {
			addCandidateOrg(candidateOrg);
		}
	}
	...
	protected void executeListener(String taskEventName, TaskListener taskListener) {
		...
		try {
			...
			throw new ActivitiException((new StringBuilder()).append("Exception while invoking TaskListener: ")
