...
public class TaskEntityManager extends AbstractManager {
	...
	public void deleteTasksByProcessInstanceId(String processInstanceId, String deleteReason, boolean cascade) {
		...
		String reason = (deleteReason == null || deleteReason.length() == 0) ? TaskEntity.DELETE_REASON_DELETED
				: deleteReason;
			...
			if (commandContext.getProcessEngineConfiguration().getEventDispatcher().isEnabled()) {
				commandContext.getProcessEngineConfiguration().getEventDispatcher()
						.dispatchEvent(ActivitiEventBuilder.createActivityCancelledEvent(
								task.getExecution().getActivityId(), task.getName(), task.getExecutionId(),
								task.getProcessInstanceId(), task.getProcessDefinitionId(), "userTask",
	...
	public void deleteTask(TaskEntity task, String deleteReason, boolean cascade) {
		if (!task.isDeleted()) {
			...
			if (cascade) {
				...
				commandContext.getHistoryManager().recordTaskEnd(task.getExecution(),taskId, deleteReason);
			}
	...
	public List<Task> findTasksAndVariablesByQueryCriteria(TaskQueryImpl taskQuery) {
		...
		// paging doesn't work for combining task instances and variables due to an
		// outer join, so doing it in-memory
		...
		List<Task> instanceList = getDbSqlSession().selectListWithRawParameterWithoutFilter(query, taskQuery,
	...
	public List<Task> findTasksByNativeQuery(Map<String, Object> parameterMap, int firstResult, int maxResults) {
		return getDbSqlSession().selectListWithRawParameter("selectTaskByNativeQuery", parameterMap, firstResult,
				maxResults);
	}
	...
	// by lw
	public void deleteTaskForce(String taskId, String deleteReason, boolean cascade) {
		TaskEntity task = Context.getCommandContext().getTaskEntityManager().findTaskById(taskId);
...
//by lw
	public void deleteTaskAndExecution(String taskId, String deleteReason, boolean cascade) {
		TaskEntity task = Context.getCommandContext().getTaskEntityManager().findTaskById(taskId);
		if (task != null) {
			String reason = (deleteReason == null || deleteReason.length() == 0) ? TaskEntity.DELETE_REASON_DELETED
					: deleteReason;
			if (!task.isDeleted()) {
				...
				HistoricActivityInstanceEntity historicActivityInstance = commandContext.getHistoryManager()
						.findActivityInstance(executionEntity);
				commandContext.getHistoricActivityInstanceEntityManager().delete(historicActivityInstance);
				commandContext.getExecutionEntityManager().delete(historicActivityInstance);
