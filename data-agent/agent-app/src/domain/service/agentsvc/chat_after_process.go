...
func (agentSvc *agentSvc) AfterProcess(ctx context.Context, callResult []byte, req *agentreq.ChatReq, agent *agentfactorydto.Agent) ([]byte, bool, error) {
	...
	// if ndata, isErr := afresvo.HandleAFErrorForChatProcess(newData); isErr {
	// 	httpErr := rest.NewHTTPError(ctx, http.StatusInternalServerError, apierr.AgentAPP_InternalError)
	// 	chatResponse.Error = httpErr
	// 	return ndata, true, nil
	// }
	...
	// //NOTE：目前只处理了文档的召回
	// docRetrievalField, graphRetrievalField, err := agentSvc.handleRetrievalField(ctx, result, markCite)
	// if err != nil {
	// 	o11y.Error(ctx, fmt.Sprintf("[AfterProcess] handle retrieval field err: %v", err))
	// 	httpErr := rest.NewHTTPError(ctx, http.StatusInternalServerError, apierr.AgentAPP_InternalError).WithErrorDetails(err.Error())
	// 	chatResponse.Error = httpErr
	// 	bytes, _ := sonic.Marshal(chatResponse)
	// 	return bytes, false, errors.Wrapf(err, "[AfterProcess] handle retrieval field err: %v", err)
	// }
