...
class KnDataQueryService(object):
    ...
    def __init__(self):
        self._basic_url = "http://{}:{}".format(
            Config.services.kn_data_query.host, Config.services.kn_data_query.port
        )
        self.headers = {}

    async def nebula_execute(self, kg_id, statements):
        api = "{}/api/kn-data-query/v1/open/custom-search/kgs/{}".format(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(api, json=payload) as response:
                if response.status != 200:
                    ...
                    logger.error(
                        "service error, api: {}, param: {}, {}".format(
                            api, payload, error_str
                        )
                    )

                res = await response.text()
                return {"code": response.status, "message": json.loads(res)}
