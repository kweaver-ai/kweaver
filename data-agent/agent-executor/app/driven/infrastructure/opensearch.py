...
from app.driven.external.embedding_client import embedding_client
...
class OpenSearchConnector(object):
    ...
    async def execute(self, url, body=None, timeout=300.0):
        ...
        async with aiohttp.ClientSession(auth=auth) as session:
            ...
            if "_msearch" in url:
                ...
                response = await session.get(
                    url, timeout=timeout, verify_ssl=False, headers=self.headers
                )
    ...
    async def search(
        ...
        if bm25_weight == 1 and phrase_match_weight == 0:
            ...
            total = min(
                max_return_num, results.get("hits", {}).get("total", {}).get("value", 0)
            )
        ...
        for hit_rank, line in enumerate(results.get("hits", {}).get("hits", [])):
            ...
            if hit_rank < 300:
                ...
                for field, singl_filde_highlight in line["highlight"].items():
                    ...
                    term_fre_and_len_score = sum(
                        len(k) * v for k, v in hash_sub_str.items()
                    )
                    shot_percent = term_fre_and_len_score / advgl
                    if (
                        shot_percent == 1
                    ):  # 这一步是为了区分属性完全匹配情况下的长度权重问题
                ...
                re_score = (
                    bm25_weight * float("{:.2f}".format(line["_score"]))
                    + phrase_match_weight * total_field_term_len
                )
        ...
        outputs["count"] = total
        outputs["entities"] = sorted_entities[
            max((page - 1) * size, 0) : min((page * size), len(sorted_entities))
        ]
        return outputs
    ...
    async def emb_msearch(
        self, query="", spacename_entity_dict=None, page=1, size=20, max_return_num=1000
    ):
        outputs = {}
        if not spacename_entity_dict:
            raise Exception("please check graph_id is correct.")
        # 问题编码
        embeddings = await embedding_client.ado_embedding([query.strip()])
        query_float_embedding = embeddings[0]
        ...
        for _space_name, _quantized_entities_items in spacename_entity_dict.items():
            if (
                _quantized_entities_items["quantized_flag"] == "0"
                or _quantized_entities_items["quantized_flag"] == 0
            ):
                # 没量化则使用float请求
        ...
        if float_requests_index:
            ...
            float_results = await self.execute(
                url=float_requests_url, body=float_requests_body
            )
        if byte_requests_index:
            ...
            byte_results = await self.execute(
                url=byte_requests_url, body=byte_requests_body
            )
        byte_recall_number = (
            byte_results.get("hits", {}).get("total", {}).get("value", 0)
        )
        float_recall_number = (
            float_results.get("hits", {}).get("total", {}).get("value", 0)
        )
        ...
        if byte_recall_number == 0:  # 没有召回结果
            ...
            byte_pd_res = pd.DataFrame(
                byte_results["hits"]["hits"], columns=["_index", "_id", "_score"]
            )
        if float_recall_number == 0:
            float_pd_res = pd.DataFrame(columns=["_index", "_id", "_score"])
        else:
            float_pd_res = pd.DataFrame(
                float_results["hits"]["hits"], columns=["_index", "_id", "_score"]
            )
        sorted_merge_pd_res = pd.concat([float_pd_res, byte_pd_res])
        sorted_merge_pd_res = sorted_merge_pd_res.sort_values(
    ...
    async def comb_search(
        ...
        if not spacename_entity_dict:
            raise Exception("please check graph_id is correct.")
        # 向量召回
        embedd_res, sorted_emb_pd_res = await self.emb_msearch(
            query, spacename_entity_dict, page, size, max_return_num
        )
        emb_recall_number = embedd_res.get("count", 0)
        # 如果组合匹配中的向量匹配结果为空，直接返回
        if emb_recall_number == 0:
            sorted_emb_pd_res = pd.DataFrame(
                columns=["_index", "_id", "_score", "rrf_norm_emb_score"]
            )
        ...
        text_recall_number = min(
            max_return_num,
            text_response.get("hits", {}).get("total", {}).get("value", 0),
        )
        # 处理文本召回结果
        if text_recall_number > 0:
            # 如果文本召回查出結果
            sorted_text_pd_res = pd.DataFrame(
                text_response["hits"]["hits"], columns=["_index", "_id", "_score"]
            )
            # #rrf归一化embedding recall 分值0-1
            sorted_text_pd_res["rrf_norm_text_score"] = 1 / (
                sorted_text_pd_res.index + 1
            )

            sorted_merge_pd_res = pd.merge(
                sorted_emb_pd_res[
                    ["_index", "_id", "_score", "rrf_norm_emb_score"]
                ].rename(
                    columns={"_score": "original_cos_sim_score", "_index": "emb_index"}
                ),
                sorted_text_pd_res[
                    ["_index", "_id", "_score", "rrf_norm_text_score"]
                ].rename(
                    columns={"_score": "original_bm25_score", "_index": "text_index"}
                ),
                on="_id",
                how="outer",
            )
            ...
            # 计算融合得分
            sorted_merge_pd_res["merge_score"] = (
                sorted_merge_pd_res["rrf_norm_emb_score"] * weight_emb
            ...
            for rank in range(
                max(0, ((page - 1) * size)),
                min(total, (max(0, (page - 1) * size) + size)),
            ):
                _line = sorted_merge_pd_res.loc[rank]
                temp = {}
                temp["vid"] = _line["_id"]
    ...
    async def create_cot_schema(self, cot_index):
        ...
        for filed in vec_field_list:
            fields[filed] = {
                "type": "knn_vector",
                "dimension": Config.external_services.embedding_dimension,
                "method": {
                    "name": "hnsw",
                ...
                if result["error"]["type"] == "resource_already_exists_exception":
                    ...
                    async with aiohttp.ClientSession(auth=auth) as session:
                        response = await session.delete(
                            url=cot_url, verify_ssl=False, headers=self.headers
                        )
                        result = await response.content.read()
                        result = json.loads(result.decode(), strict=False)
                        if response.status != 200:
    ...
    async def inseart_cot_data(self, cot_index, cot_contents: list):
        ...
        for cot_content in cot_contents:
            ...
            # 编码
            embedding_ls = await embedding_client.ado_embedding(
                [question, question_skeleton]
            )
    ...
    async def emb_cot_search(self, cot_index, query, field_name: str):
        ...
        requests_body = {
            # "_source": [""],
            "query": {"knn": {field_name: {"vector": query_float_embedding, "k": 50}}},
            "size": 20,
        }
        search_response = await self.execute(
            url=cot_index + "/_search", body=requests_body
        )
    ...
    async def create_index(self, index_name: str, index_body: dict):
        ...
        async with aiohttp.ClientSession(auth=auth) as session:
            ...
            result = json.loads(result.decode(), strict=False)
            if response.status != 200:
                err = self.pre_url + " create_index error: {}".format(
                    await response.text()
                )
    ...
    async def is_index_exists(self, index_name: str):
        ...
        async with aiohttp.ClientSession(auth=auth) as session:
            response = await session.get(
                index_url, timeout=timeout, verify_ssl=False, headers=self.headers
            )
            if response.status == 200:
                return True
            else:
    ...
    async def insert_data(self, index_name: str, data: dict, doc_id=None):
        ...
        async with aiohttp.ClientSession(auth=auth) as session:
            response = await session.post(
                url=insert_url, json=data, verify_ssl=False, headers=self.headers
            )
            result = await response.content.read()
            res_json = json.loads(result.decode(), strict=False)
            if response.status not in [200, 201]:
                err = self.pre_url + " insert_data error: {}".format(
                    await response.text()
                )
                error_log = log_oper.get_error_log(err, sys._getframe())
                StandLogger.error(error_log, log_oper.SYSTEM_LOG)
    ...
    async def delete_by_query(self, index_name: str, query: dict):
        ...
        async with aiohttp.ClientSession(auth=auth) as session:
            response = await session.post(
                url=delete_url, json=query, verify_ssl=False, headers=self.headers
            )
            result = await response.content.read()
            res_json = json.loads(result.decode(), strict=False)
            if response.status != 200:
                err = self.pre_url + " delete_by_query error: {}".format(
                    await response.text()
                )
                error_log = log_oper.get_error_log(err, sys._getframe())
                StandLogger.error(error_log, log_oper.SYSTEM_LOG)
    ...
    async def get_doc_by_ids(self, index_name: str, doc_ids: list) -> list:
        ...
        async with aiohttp.ClientSession(auth=auth) as session:
            response = await session.get(
                url=url, json=body, verify_ssl=False, headers=self.headers
            )
            if response.status != 200:
                err = self.pre_url + " get_doc_by_ids error: {}".format(
                    await response.text()
                )
                error_log = log_oper.get_error_log(err, sys._getframe())
                StandLogger.error(error_log, log_oper.SYSTEM_LOG)
...
opensearch_engine = OpenSearchConnector(
    ips=Config.opensearch.host.split(","),
    ports=Config.opensearch.port.split(","),
