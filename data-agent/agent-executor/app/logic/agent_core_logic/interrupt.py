...
class InterruptHandler:
    ...
    async def handle_tool_interrupt(
        ...
        if span and span.is_recording():
            span.set_attribute("session_id", context_variables.get("session_id", ""))
            span.set_attribute("agent_id", context_variables.get("agent_id", ""))
        StandLogger.info(f"ToolInterrupt: {tool_interrupt}")

        context_variables = res.get("answer", {})
