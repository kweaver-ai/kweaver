...
from app.common.structs import DEFAULT_INPUTS, AgentConfig
...
class PromptBuilder:
    def __init__(self, agent_config: AgentConfig, temp_files: dict[str, list]):
    ...
    async def build(self, span: Optional[Span] = None) -> str:
        if span and span.is_recording():
            span.set_attribute("session_id", self.agent_config.session_id)
            span.set_attribute("agent_id", self.agent_config.agent_id)
    ...
    @internal_span()
    async def get_tool_names(self, span: Span = None) -> list[str]:
        """获取工具名称"""
        if span and span.is_recording():
            span.set_attribute(
                "session_id",
                self.agent_config.session_id if self.agent_config.session_id else "",
            )
            span.set_attribute(
                "agent_id",
        ...
        if self.agent_config.skills:
            for tool in self.agent_config.skills.tools:
            ...
            for agent in self.agent_config.skills.agents:
                tool_info = await agent_factory_service.get_agent_config_by_key(
                    agent.agent_key
                )
                tool_names.add(tool_info["name"])

            for mcp in self.agent_config.skills.mcps:
                tool_info = await agent_operator_integration_service.get_mcp_tools(
                    mcp.mcp_server_id
                )
...
/end/\n"""
    ...
    def search_memory_prompt(self, span: Span = None) -> str:
        ...
        inputs_prompt = ""
        for input_field in self.agent_config.input.get("fields", []):
            if (
                input_field.get("type") != "file"
                and input_field.get("name") not in DEFAULT_INPUTS
            ):
                inputs_prompt += (
                    f"{input_field.get('name')}: ${input_field.get('name')}\n"
                )

        memory_prompt = f"{repr(inputs_prompt)} -> all_inputs\n"
        memory_prompt += f"""@search_memory(query=$all_inputs, user_id=$header['x-account-id'], limit={Config.memory.limit}, threshold={Config.memory.threshold}, rerank_thread={Config.memory.rerank_threshold}) -> relevant_memories\n"""
        memory_prompt += """$history + [{"role": "user", "content": "Memories:\\n" + "\\n".join(f"- {entry['memory']}" for entry in $relevant_memories["answer"]["result"])}] -> history\n"""
        memory_prompt += "'' -> all_inputs\n"
