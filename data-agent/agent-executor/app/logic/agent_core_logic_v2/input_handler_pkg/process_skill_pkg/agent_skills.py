...
async def process_skills_agents(
    ...
    for agent_skill_cfg in skills.agents:
        ...
        # 1. 从session缓存中获取agent_info
        agent_info = ac.session_handler.get_skill_agent_info_dict(
            agent_skill_cfg.agent_key
        )

        # 2. 如果session缓存中没有agent_info, 则从agent_factory_service获取
        if not agent_info:
            ...
            if ac.is_warmup:
                ac.session_handler.set_skill_agent_info_dict(
