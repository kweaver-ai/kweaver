...
from app.domain.enum.common.user_account_header_key import set_user_account_id, set_user_account_type
...
from data_migrations.init.built_in_agent_handler_pkg.agent_publisher import publish_agent
from data_migrations.init.built_in_agent_handler_pkg.config_loader import load_agent_config
...
def process_agents(UPDATE_MODE):
    ...
    for filename in os.listdir(agents_dir):
        if filename.endswith(".py") and not filename.startswith("__"):
            ...
            # 2. 创建或更新agent
            agent_id = create_agent(agent_data, API_BASE_URL, headers, user_id, UPDATE_MODE)
            ...
            # 3. 发布agent
            if UPDATE_MODE:
                publish_agent(agent_id, API_BASE_URL, headers, user_id, agent_data["description"])
...
def main():
    ...
    for idx, interval in enumerate(retry_intervals):
        try:
            print(f"运行模式: {'更新' if UPDATE_MODE else '初始化'}（第 {idx + 1} 次尝试）")
            # 1. 插入内置工具
