...
#     f_seq_no          bigint       not null auto_increment comment '自增序号' unique,
    ...
    f_is_built_in     tinyint(1)   not null default 0 comment '是否是内置的，枚举值：0-否，1-是',
    f_is_system_agent tinyint(1)   not null default 0 comment '是否是系统agent，枚举值：0-否，1-是',

    f_product_key     varchar(40)  not null default '' comment '产品标识',

    f_avatar_type     tinyint(1)   not null default 0 comment '头像类型，0-内置头像，1-用户上传头像，2-AI生成头像',
...
#     f_is_last_one       tinyint(1)   not null default 1 comment '是否是最后一个（说明： 对于同一个模板（f_key相同） 1. 如果是未发布的，只有一个，这个就是最后一个；2. 如果是已发布的，并且发布后没有新的编辑，这个就是最后一个；3. 如果是已发布的，并且发布后有新的编辑，此时会有两条key一样的记录。已发布的这个不是最后一个，最新编辑的这个才是最后一个。 此字段用于在“我的模板列表”中，展示f_is_last_one=1的记录）枚举值：0-否，1-是',
...
#     f_seq_no             bigint       not null auto_increment comment '自增序号' unique,
    ...
    f_is_api_agent       tinyint(1)   not null default 0 comment '是否发布为api agent，枚举值：0-否，1-是',
    f_is_web_sdk_agent   tinyint(1)   not null default 0 comment '是否发布为web sdk agent，枚举值：0-否，1-是',
    f_is_skill_agent     tinyint(1)   not null default 0 comment '是否发布为技能 agent，枚举值：0-否，1-是',
    f_is_data_flow_agent tinyint(1)   not null default 0 comment '是否发布为数据流 agent，枚举值：0-否，1-是',

    f_is_to_custom_space tinyint(1)   not null default 0 comment '是否发布到自定义空间，枚举值：0-否，1-是',
    f_is_to_square       tinyint(1)   not null default 0 comment '是否发布到广场，枚举值：0-否，1-是',

    f_is_pms_ctrl        tinyint(1)   not null default 0 comment '是否进行“使用权限”的管控，枚举值：0-否，1-是。开启管控是，会在t_data_agent_release_permission表中记录哪些对象有权限使用此data agent',
