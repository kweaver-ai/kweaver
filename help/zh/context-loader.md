...
除了工具，Context Loader 还提供 MCP 资源和提示词模板：

```bash
# 查看可用资源（如 kn://kn_abc123/object-types）
...
# 列出 URI 模板
kweaver context-loader templates
...
# Schema 搜索
curl -sk -X POST "$KWEAVER_BASE/api/agent-retrieval/v1/kn-search" \
  -H "Authorization: Bearer $TOKEN" \
...
# 查询对象实例
curl -sk -X POST "$KWEAVER_BASE/api/agent-retrieval/v1/query-object-instance" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "kn_id": "kn_abc123",
...
# 查询实例子图
curl -sk -X POST "$KWEAVER_BASE/api/agent-retrieval/v1/query-instance-subgraph" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "kn_id": "kn_abc123",
...
# 获取逻辑属性
curl -sk -X POST "$KWEAVER_BASE/api/agent-retrieval/v1/get-logic-properties" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "kn_id": "kn_abc123",
...
# 获取动作信息
curl -sk -X POST "$KWEAVER_BASE/api/agent-retrieval/v1/get-action-info" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "kn_id": "kn_abc123",
