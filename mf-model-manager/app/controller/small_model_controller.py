...
async def edit_model(request: logics.EditExternalSmallModel, userId, language, role):
    try:
        ...
        try:
            ...
            cache_key = f"dip:model-manager:small-model:{request.model_name}:list"
            global redis_util
    ...
    try:
        ...
        try:
            ...
            for model_info in original_res:
                ...
                cache_key_list.append(f"dip:model-manager:small-model:{model_name}:list")
            global redis_util
            if redis_util is None:
                redis_util = await get_redis_util()
...
async def embedding_model_used(request, userId, language, role, func_module, private=True):
    try:
        ...
        texts = request.input
        cache_key = f"dip:model-manager:small-model:{model_name}:list"
        # 确保 redis_util 已初始化
        global redis_util
        if redis_util is None:
            redis_util = await get_redis_util()
