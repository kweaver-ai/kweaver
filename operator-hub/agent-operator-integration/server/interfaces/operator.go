...
type OperatorRegisterReq struct {
	MetadataType           MetadataType            `json:"operator_metadata_type" form:"operator_metadata_type" validate:"required" oneof:"openapi"` // 算子元数据类型(强制参数)
	Data                   string                  `json:"data" form:"data" validate:"required"`                                                     // 算子元数据(强制参数)
...
type OperatorEditReq struct {
	...
	MetadataType           MetadataType            `json:"metadata_type" form:"metadata_type" validate:"omitempty,oneof=openapi"` // 元数据类型(可选参数)
...
type OperatorDataInfo struct {
	...
	MetadataType           MetadataType            `json:"metadata_type" default:"openapi" validate:"oneof=openapi"`              // 算子元数据类型(强制参数)
...
type ExecuteOperatorReq struct {
	...
	Headers     map[string]string `json:"header"`                               // 请求头
	Body        interface{}       `json:"body"`                                 // 请求体
	QueryParams map[string]string `json:"query"`                                // 查询参数
	PathParams  map[string]string `json:"path"`                                 // 路径参数
...
type OperatorManager interface {
	...
	GetOperatorInfoByOperatorID(ctx context.Context, operatorID string) (*OperatorDataInfo, error)
	GetOperatorCategoryList(ctx context.Context) ([]*CategoryInfo, error)
	...
	// 内部接口
	GetOperatorReleaseByIDPrivate(ctx context.Context, operatorID string) (*OperatorDataInfo, error)
	GetOperatorMetadataVersionByIDs(ctx context.Context, operatorIDs []string) (metadataMap map[string]string, err error)
	// GetOperatorInfoByOperatorIDPrivate 获取算子信息
	GetOperatorInfoByOperatorIDPrivate(ctx context.Context, operatorID string) (*OperatorDataInfo, error)
	/*导入导出*/
...
type RegisterInternalOperatorReq struct {
	...
	OperatorID             string                  `json:"operator_id" form:"operator_id" validate:"required,uuid4"`                            // 算子ID
	MetadataType           MetadataType            `json:"metadata_type" form:"metadata_type" validate:"required" oneof:"openapi"`              // 算子元数据类型(强制参数)
	Data                   json.RawMessage         `json:"data" form:"data" validate:"required"`                                                // 算子元数据(强制参数)
