...
func (s *mcpServiceImpl) Import(ctx context.Context, tx *sql.Tx, mode interfaces.ImportType, data *interfaces.ComponentImpexConfigModel, userID string) (err error) {
	...
	createMap, updateMap, mcpToolsMap, depToolBoxMap, err := s.batchImportMcpMetadata(ctx, tx, data.MCP.Configs, waitUpdataMCPList, accessor)
	if err != nil {
		return
	}
	// 导入依赖
	...
	// 添加自定义工具记录，部署同步MCP Server信息
	for mcpID, mcpDB := range createMap {
		if mcpDB.CreationType != string(interfaces.MCPCreationTypeToolImported) {
			continue
		}
		if mcpTools, ok := mcpToolsMap[mcpID]; ok {
			_ = s.createMCPServerInstance(ctx, mcpDB, mcpTools)
		}
	}
	for mcpID, mcpDB := range updateMap {
		if mcpDB.CreationType != string(interfaces.MCPCreationTypeToolImported) {
			continue
		}
		if mcpTools, ok := mcpToolsMap[mcpID]; ok {
			_ = s.updateMCPServerInstance(ctx, mcpDB, mcpTools)
		}
	}
...
func (s *mcpServiceImpl) batchImportMcpMetadata(ctx context.Context, tx *sql.Tx, items []*interfaces.MCPServersImpexItem, waitUpdataMCPList []*model.MCPServerConfigDB,
	accessor *interfaces.AuthAccessor) (createMCPMap, updateMCPMap map[string]*model.MCPServerConfigDB, mcpToolsMap map[string][]*model.MCPToolDB, depToolBoxMap map[string]bool, err error) {
	// 收集需要新增的mcp
	...
	// 检查是否存在依赖工具
	mcpToolsMap = map[string][]*model.MCPToolDB{}
	...
	for _, item := range items {
		...
		if mcpTools != nil {
			mcpToolsMap[item.MCPID] = mcpTools
			// 记录依赖工具
...
func (s *mcpServiceImpl) batchGetExportMetadata(ctx context.Context, mcpConfigDBs []*model.MCPServerConfigDB) (config *interfaces.MCPImpexConfig,
	...
	// 依赖的工具
	mcpToolsMap := map[string]map[string]*interfaces.MCPToolItem{}
	// 收集需要获取依赖工具的MCP信息
	mcpIDs := []string{}
	for _, configDB := range mcpConfigDBs {
		// 内置MCP不允许导出
		...
		if configDB.CreationType == interfaces.MCPCreationTypeToolImported.String() {
			mcpIDs = append(mcpIDs, configDB.MCPID)
			mcpConfig.MCPTools = []*interfaces.MCPToolItem{}
			mcpToolsMap[configDB.MCPID] = map[string]*interfaces.MCPToolItem{}
		}
	...
	if len(mcpIDs) > 0 {
		...
		mcpToolDBs, err = s.DBMCPTool.SelectListByMCPIDS(ctx, nil, mcpIDs)
		if err != nil {
		...
		for _, mcpToolDB := range mcpToolDBs {
			...
			mcpToolsMap[mcpToolDB.MCPID][mcpToolDB.MCPToolID] = &interfaces.MCPToolItem{
		...
		for _, mcpServerConfig := range config.Configs {
			if mcpServerConfig.CreationType != interfaces.MCPCreationTypeToolImported {
				continue
			}
			if mcpToolsMap[mcpServerConfig.MCPID] == nil {
				continue
			}
			for _, mcpTool := range mcpToolsMap[mcpServerConfig.MCPID] {
				mcpServerConfig.MCPTools = append(mcpServerConfig.MCPTools, mcpTool)
			}
		}
	}
	return
