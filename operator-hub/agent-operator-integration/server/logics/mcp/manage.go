...
func (s *mcpServiceImpl) DeleteMCPServer(ctx context.Context, req *interfaces.MCPServerDeleteRequest) (err error) {
	...
	if configDB.CreationType == interfaces.MCPCreationTypeToolImported.String() {
		...
		// 删除mcp Server实例
		err = s.removeMCPServerInstance(ctx, req.MCPID, configDB.Version)
		if err != nil {
			return err
		}
	}
...
func (s *mcpServiceImpl) updateMCPConfigVersion(ctx context.Context, tx *sql.Tx, mcpConfigDB *model.MCPServerConfigDB) (version int, err error) {
	...
	return mcpConfigDB.Version, nil
}
...
// removeMCPServerInstance 删除MCP Server实例
func (s *mcpServiceImpl) removeMCPServerInstance(ctx context.Context, mcpID string, mcpVersion int) (err error) {
	// 删除mcp Server实例
	err = s.AgentOperatorApp.DeleteMCPInstance(ctx, mcpID, mcpVersion)
	if err != nil {
		return err
	}
	return nil
...
func (s *mcpServiceImpl) convertInputSchema(ctx context.Context, toolInfo *interfaces.ToolInfo) (json.RawMessage, error) {
	if toolInfo.MetadataType != interfaces.MetadataTypeAPI {
		...
		return nil, fmt.Errorf("unsupported metadata type: %s", toolInfo.MetadataType)
	}
	...
	err := utils.AnyToObject(toolInfo.Metadata, metadata)
	if err != nil {
		s.logger.WithContext(ctx).Warnf("tool metadata is not apimetadata, err: %v", err)
		return nil, err
	}
	...
	if err := utils.AnyToObject(metadata.APISpec, apiSpec); err != nil {
		s.logger.WithContext(ctx).Warnf("tool apispec is not apispec, err: %v", err)
		return nil, err
	}
	converter := NewSimpleConverter()
	result := converter.ConvertFromBytes(utils.ObjectToByte(apiSpec))
	if !result.Success {
