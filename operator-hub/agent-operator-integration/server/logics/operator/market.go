...
// GetOperatorReleaseByIDPrivate 算子市场查询操作（私有）
func (m *operatorManager) GetOperatorReleaseByIDPrivate(ctx context.Context, operatorID string) (result *interfaces.OperatorDataInfo, err error) {
	// 记录可观测
	...
	// 获取元数据信息
	exist, metadataDB, err := m.DBAPIMetadataManager.SelectByVersion(ctx, releaseDB.MetadataVersion)
	if err != nil {
		m.Logger.WithContext(ctx).Errorf("query metadata failed, err: %v", err)
		err = errors.DefaultHTTPError(ctx, http.StatusInternalServerError, "query metadata failed")
		return
	}
	if !exist {
	...
	_, result, err = m.assembleReleaseResult(ctx, releaseDB, metadataDB)
	return
}
...
func (m *operatorManager) QueryOperatorMarketList(ctx context.Context, req *interfaces.PageQueryOperatorMarketReq) (
	...
	for _, release := range releaseList {
		metadataVersions = append(metadataVersions, release.MetadataVersion)
	}
	// 获取元数据信息
	metadataMap, err := m.getAPIMetadataMap(ctx, metadataVersions)
	if err != nil {
		return
	}
	// 组装算子信息结果
	...
	for _, release := range releaseList {
		...
		metadataDB, ok := metadataMap[release.MetadataVersion]
		if !ok {
...
// 根据请求参数查询并过滤算子发布列表
func (m *operatorManager) queryOperatorReleaseList(ctx context.Context, req *interfaces.PageQueryOperatorMarketReq) (authResp *interfaces.QueryResponse[model.OperatorReleaseDB], resourceToBdMap map[string]string, err error) {
...
func (m *operatorManager) getAPIMetadataMap(ctx context.Context, metadataVersions []string) (metadataMap map[string]*model.APIMetadataDB, err error) {
	metadataMap = map[string]*model.APIMetadataDB{}
	metadataList, err := m.DBAPIMetadataManager.SelectListByVersion(ctx, metadataVersions)
	if err != nil {
		m.Logger.WithContext(ctx).Warnf("select operator metadata failed, MetadataVersions: %v, err: %v", metadataVersions, err)
		err = errors.DefaultHTTPError(ctx, http.StatusInternalServerError, "select operapi metadata failed")
		return
	}
