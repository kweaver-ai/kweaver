...
func (m *operatorManager) RegisterOperatorByOpenAPI(ctx context.Context, req *interfaces.OperatorRegisterReq, userID string) (resultList []*interfaces.OperatorRegisterResp, err error) {
	...
	// 解析API文档
	items, err := m.checkAndParserOpenAPIOperator(ctx, req)
	if err != nil {
		return
	}
	// 初始化算子注册状态
	...
	// 只允许单个算子直接注册发布
	if req.DirectPublish && len(items) > 1 {
		...
		return
	} else if req.DirectPublish && len(items) == 1 {
		operatorRegisterStatus = interfaces.BizStatusPublished
	}
	resultList = []*interfaces.OperatorRegisterResp{}
	// 遍历算子列表，解析元数据
	for _, item := range items {
		resultList = append(resultList, m.registerOperator(ctx, req, item, accessor, operatorRegisterStatus, isDataSource))
	}
...
func (m *operatorManager) UpdateOperatorByOpenAPI(ctx context.Context, req *interfaces.OperatorUpdateReq, userID string) (resultList []*interfaces.OperatorRegisterResp, err error) {
	...
	// 解析API文档
	items, err := m.checkAndParserOpenAPIOperator(ctx, req.OperatorRegisterReq)
	if err != nil {
	...
	operator, metadata, accessor, err := m.preCheckEdit(ctx, updateReq, req.DirectPublish)
	if err != nil {
		m.Logger.WithContext(ctx).Warnf("[UpdateOperatorByOpenAPI] pre check edit failed, err: %v", err)
		return
	}
	editMetdataParam, err := m.buildUpdateMetdataParam(ctx, item)
	if err != nil {
		m.Logger.WithContext(ctx).Warnf("build update metdata param failed, err: %v", err)
		return
	}
	editRes, err := m.editOperator(ctx, updateReq, operator, metadata, editMetdataParam, req.DirectPublish, isDataSource)
	if err != nil {
...
func (m *operatorManager) validateOperator(ctx context.Context, item *interfaces.PathItemContent) (err error) {
	if item.ErrMessage != "" {
		err = errors.DefaultHTTPError(ctx, http.StatusBadRequest, item.ErrMessage)
		return
	}
	// 校验算子名称
	...
	// 校验算子描述
	err = m.Validator.ValidateOperatorDesc(ctx, item.Description)
	return
}

// checkAndParserOpenAPIOperator 检查并解析OpenAPI算子
func (m *operatorManager) checkAndParserOpenAPIOperator(ctx context.Context, req *interfaces.OperatorRegisterReq) (items []*interfaces.PathItemContent, err error) {
	// 检查算子类型
	...
	// 解析API数据
	items, err = m.OpenAPIParser.GetPathItems(ctx, []byte(req.Data))
	if err != nil {
		m.Logger.WithContext(ctx).Infof("parse operator failed, err: %v", err.Error())
		return
	}
	// 检查Items长度
	err = m.Validator.ValidateOperatorImportCount(ctx, int64(len(items)))
	if err != nil {
		return
	}
	return
...
func (m *operatorManager) registerOperator(ctx context.Context, req *interfaces.OperatorRegisterReq,
	item *interfaces.PathItemContent, accessor *interfaces.AuthAccessor, status interfaces.BizStatus, isDataSource bool) (result *interfaces.OperatorRegisterResp) {
	// 记录可观测
	ctx, _ = o11y.StartInternalSpan(ctx)
	...
	// 2. 插入元数据
	version, err := m.DBAPIMetadataManager.InsertAPIMetadata(ctx, tx, metadata)
	if err != nil {
