...
async def add_model(schema_para, userId, language):
    ...
    if userId != "266c6a42-6131-4d62-8f39-853e7093701c":
        schema_para["quota"] = False  # 非admin这一项均为false
    ...
    if content:
        ...
        try:
            ...
            llm_model_dao.add_data_into_model_list(model_id, schema_para["model_series"],
                                                   ...
                                                   schema_para.get("model_parameters", None), schema_para["quota"])
            content = {"status": "ok", "id": model_id}
            if schema_para["quota"] is False:
                # 需要预插入一条模型配额
...
async def remove_model(model_ids, userId, language):
    try:
        ...
        for model_id in model_ids["model_ids"]:
            ...
            cache_key_list.append(f"dip:model-manager:llm:{model_name}:list")
...
async def test_model(model_config, userId, language):
    if 'model_id' in model_config:
        try:
            info = llm_model_dao.get_data_from_model_list_by_id(model_config["model_id"])
            if not info:
    ...
    if llm_edit_verify(model_para):
        ...
        try:
            ...
            if model_para['model_id'] not in ids_list:
                ...
                model_config = model_para["model_config"]
                ...
                if re_name not in model_name_list or re_name == old_name:
                    ...
                    if redis_util is None:
                        redis_util = await get_redis_util()
                    quota_cache_key = f"dip:model-manager:llm:{model_name}:list"
                    llm_cache_key = f"dip:model-manager:llm:{model_name}:list"
...
async def used_model_openai(request, user_id, language, func_module):
    ...
    cache_key = f"dip:model-manager:llm:{model_name}:list"
    try:
        # 确保 redis_util 已初始化
        global redis_util
        if redis_util is None:
    ...
    if quota:
        quota_cache_key = f"dip:model-manager:llm-quota:{model_name}:list"
...
async def edit_default_model(model_para, userId, language):
    ...
    try:
        ...
        default_cache_key = f"dip:model-manager:llm:{default_cache_name}:list"
        global redis_util
        if redis_util is None:
            redis_util = await get_redis_util()
