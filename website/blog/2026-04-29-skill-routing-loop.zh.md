...
**2. BKN：业务真相到 AI 可读图状态的桥。** `material` / `supplier` / `skills` 三个 ObjectType 把 MySQL 业务表 live-read 成图；`applicable_skill` 这条 direct-mapping 关系把 Skill 绑定到具体物料——`materials.bound_skill_id` 改一下，这条边就跟着变。这一层回答的是「哪个物料该用哪个能力」，把 Skill 路由从 prompt 工程问题彻底搬到了数据治理问题。业务系统的 SQL UPDATE 和 AI 看到的图状态，第一次有了清晰的因果链。
...
**4. Agent：有边界的编排者。** Decision Agent 的 system_prompt 里**不出现任何具体 Skill 名**，只规定流程：拿候选 → 查证据 → 选 Skill → 加载契约 → 执行 → 给依据。它不知道也不需要知道业务里"哪个物料该用哪个 Skill"——那是 BKN 的事；它也不直接读 MySQL——那是 Vega + context-loader 的事。它只在 context-loader 给的候选集里挑，再用 `builtin_skill_load` / `builtin_skill_execute_script` 把 Skill 落地。这一层回答的是「该怎么思考」，并且**只回答这一个问题**。
...
**用户在这个 example 里只需要写：3 个 Skill 包 + 5 张 BKN 模板 + 一份不到 300 字的 `agent.json` + 一份业务数据 CSV。** 剩下的复杂性——Skill 的注册 / 版本化 / 上下线、业务表到图的双向桥、Agent 与 KN / Skill 之间的协议、mode=react 的工具挂载、Decision 链的可观察、清理回滚——全部由产品来扛。
